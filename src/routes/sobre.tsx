      ...
      <section className="px-6 lg:px-12 py-24 border-t border-border">
        <div className="mx-auto max-w-[1400px]">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
            <StaggerReveal staggerDelay={100} itemClassName="h-full">
              {timeline.map((t) => (
                <div key={t.y} className="h-full flex flex-col border border-border bg-card p-8 rounded-sm transition-all duration-500 hover:border-[var(--primary)] hover:-translate-y-1">
                  <div className="font-display font-black text-5xl text-[var(--primary)] leading-none">{t.y}</div>
                  <h3 className="font-display font-bold text-xl md:text-2xl mt-4">{t.t}</h3>
                  <p className="text-muted-foreground mt-3 text-sm md:text-base">{t.d}</p>
                </div>
              ))}
            </StaggerReveal>
          </div>
